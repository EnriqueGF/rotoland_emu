	...
	internal sealed class SaveRoomSettingsMessageEvent : Interface
	{
		public void Handle(GameClient Session, ClientMessage Event)
		{
			...
			if (@class != null && @class.method_27(Session, true))
			{
				...
				string text3 = Phoenix.FilterString(Event.PopFixedString());
				...
				for (int i = 0; i < num4; i++)
				{
					...
					string text4 = Phoenix.FilterString(Event.PopFixedString().ToLower());
				...
				if (!(text != ChatCommandHandler.smethod_4(text)) && !(text2 != ChatCommandHandler.smethod_4(text2)) && text.Length >= 1 && (num9 >= -2 && num9 <= 1 && num10 >= -2 && num10 <= 1))
				{
					...
					{
						FlatCat class2 = Phoenix.GetGame().GetNavigator().method_2(num3);
						if (class2 != null)
						{

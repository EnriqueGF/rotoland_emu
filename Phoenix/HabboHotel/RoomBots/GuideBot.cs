...
namespace Phoenix.HabboHotel.RoomBots
{
	internal sealed class GuideBot : BotAI
	{
		...
		{
			base.GetRoomUser().method_1(null, PhoenixEnvironment.smethod_1("guidebot_welcome1"), true);
			base.GetRoomUser().method_1(null, PhoenixEnvironment.smethod_1("guidebot_welcome2"), false);
		}
		...
		public override void OnUserSay(RoomUser RoomUser_0, string string_0)
		{
			if (base.method_1().method_100(base.GetRoomUser().int_3, base.GetRoomUser().int_4, RoomUser_0.int_3, RoomUser_0.int_4) <= 8)
			{
				...
				if (@class != null)
				{
					...
					if (@class.ServeId >= 1)
					{
						RoomUser_0.method_8(@class.ServeId);
					}
				}
			}
		}

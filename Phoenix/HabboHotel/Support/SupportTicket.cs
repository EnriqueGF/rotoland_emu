...
namespace Phoenix.HabboHotel.Support
{
	internal sealed class SupportTicket
	{
		...
		public SupportTicket(uint mId, int mScore, int mType, uint mSenderId, uint mReportedId, string mMessage, uint mRoomId, string mRoomName, double mTimestamp, uint mModeratorId)
		{
			...
			this.string_2 = Phoenix.GetGame().GetClientManager().GetNameById(mSenderId);
			this.string_3 = Phoenix.GetGame().GetClientManager().GetNameById(mReportedId);
			this.string_4 = Phoenix.GetGame().GetClientManager().GetNameById(mModeratorId);
		}
		public void method_0(uint ModeratorId, bool UpdateInDb)
		{
			this.Status = TicketStatus.PICKED;
			this.ModeratorId = ModeratorId;
			this.string_4 = Phoenix.GetGame().GetClientManager().GetNameById(ModeratorId);
			if (UpdateInDb)
			{

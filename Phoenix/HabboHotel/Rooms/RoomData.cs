...
namespace Phoenix.HabboHotel.Rooms
{
	class RoomData
	{
		...
		public void FillNull(uint mId)
		{
			...
			this.class28_0 = Phoenix.GetGame().GetRoomManager().GetModel(ModelName, mId);
		}
		public void method_1(DataRow Row)
		{
			...
			this.Tags = new List<string>();
			this.AllowPet = Phoenix.smethod_3(Row["allow_pets"].ToString());
			this.AllowPetsEating = Phoenix.smethod_3(Row["allow_pets_eat"].ToString());
			this.AllowWalkthrough = Phoenix.smethod_3(Row["allow_walkthrough"].ToString());
			this.bool_3 = false;
			this.Hidewall = Phoenix.smethod_3(Row["allow_hidewall"].ToString());
			...
			this.class28_0 = Phoenix.GetGame().GetRoomManager().GetModel(this.ModelName, this.Id);
		}
		public void Fill(Room Room)
		{
			...
			this.class28_0 = Phoenix.GetGame().GetRoomManager().GetModel(ModelName, Id);
		}
		public void method_3(ServerMessage Message, bool ShowEvents, bool bool_6)
		{
			...
			if (Event == null || !ShowEvents)
			{
				...
				if (this.Category > 0)
				{
					Message.AppendBoolean(Phoenix.GetGame().GetNavigator().method_2(Category).CanTrade);
				}
				else
				{
					Message.AppendBoolean(false);

...
namespace Phoenix.HabboHotel.Rooms
{
	internal sealed class RoomManager
	{
		...
		{
			string_0 = Phoenix.FilterString(string_0);
			RoomData result;
			...
			{
				if (this.Models[string_1].bool_0 && !Session.GetHabbo().method_20().method_2("habbo_club") && !Session.GetHabbo().method_20().method_2("habbo_vip"))
				{

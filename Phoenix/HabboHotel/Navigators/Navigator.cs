	...
	internal sealed class Navigator
	{
		...
		public void method_0(DatabaseClient class6_0)
		{
			...
			if (dataTable != null)
			{
				...
				{
					this.list_0.Add(new FlatCat((int)dataRow["Id"], (string)dataRow["caption"], (int)dataRow["min_rank"], Phoenix.smethod_3(dataRow["cantrade"].ToString())));
				}
			}
			if (dataTable2 != null)
			{
		...
		public ServerMessage method_6(GameClient Session)
		{
			...
			using (TimedLock.Lock(Session.GetHabbo().list_1))
			{
				...
				{
					Phoenix.GetGame().GetRoomManager().method_11(current).method_3(Message, false, false);
				}
			}
			return Message;
		}
		...
		public ServerMessage method_10(string string_0)
		{
			...
			using (DatabaseClient @class = Phoenix.GetDatabase().GetClient())
			{
				string_0 = Phoenix.FilterString(string_0.ToLower()).Trim();
